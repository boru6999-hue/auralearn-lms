...
export default function Navbar() {
  ...
  useEffect(() => {
    const h = (e: MouseEvent) => {
      ...
      if (!t.closest(".nav-lang-wrap")) setLang(false);
      if (!t.closest(".nav-user-wrap")) setUser(false);
    };
  ...
  const ABR   = isDark ? "rgba(255,255,255,0.12)"  : "rgba(0,0,0,0.1)";
  const DROP  = isDark ? "#111"                    : "#fff";
  const DROPB = isDark ? "rgba(255,255,255,0.07)"  : "rgba(0,0,0,0.07)";
  const DH    = isDark ? "rgba(255,255,255,0.05)"  : "rgba(0,0,0,0.04)";
  const SHADOW = isDark ? "0 8px 32px rgba(0,0,0,0.7)" : "0 8px 32px rgba(0,0,0,0.12)";
      ...
      <nav style={{
        ...
        <div style={{
          ...
          <Link href="/" style={{
            ...
            <span style={{ fontSize: "12px", fontWeight: 600, color: TEXT, whiteSpace: "nowrap" }}>AuraLearn</span>
          </Link>

          {/* Links */}
          <div className="nl" style={{ display: "flex", alignItems: "center", gap: "1px" }}>
            {LINKS.map(l => (
              <Link key={l.href} href={l.href} style={{
                padding: "6px 12px", borderRadius: "100px", fontSize: "12px",
                color: (l as any).gold?"rgba(181,134,58,0.85)":active(l.href)?TEXT:MUTED,
                background: (l as any).gold?"rgba(181,134,58,0.08)":active(l.href)?ABG:"transparent",
                border: `1px solid ${(l as any).gold?"rgba(181,134,58,0.18)":active(l.href)?ABR:"transparent"}`,
          ...
          {/* Divider */}
          <div className="nl" style={{ width: "1px", height: "16px", background: PILLB, margin: "0 3px", flexShrink: 0 }} />

          {/* Lang */}
          <div className="nav-lang-wrap" style={{ position: "relative" }}>
            <button onClick={() => { setLang(!langOpen); setUser(false); }} style={{
              padding: "6px 9px", borderRadius: "100px", fontSize: "12px",
              color: MUTED, background: "none", border: "none", cursor: "pointer",
              display: "flex", alignItems: "center", gap: "4px",
              fontFamily: "inherit", whiteSpace: "nowrap",
            }}>
              {curLang?.flag}
              <i className={`fa-solid fa-chevron-${langOpen?"up":"down"}`} style={{ fontSize: "8px" }} />
            </button>
            {langOpen && (
              <div style={{ ...dropStyle, right: 0, minWidth: "150px", maxHeight: "300px", overflowY: "auto" }}>
                {LANGS.map(l => (
                  <button key={l.code} onClick={() => changeLang(l.code)} style={{
                    width: "100%", background: lang===l.code?DH:"transparent",
                    border: "none", color: TEXT, padding: "7px 12px", borderRadius: "8px",
                    cursor: "pointer", fontSize: "12px", textAlign: "left",
                    display: "flex", alignItems: "center", gap: "8px",
                    fontFamily: "inherit", fontWeight: lang===l.code?500:300,
                  }}>
                    {l.flag} {l.label}
                  </button>
                ))}
          ...
          {session ? (
            <div className="nav-user-wrap" style={{ position: "relative" }}>
              <button onClick={() => { setUser(!userOpen); setLang(false); }} style={{
                width: "28px", height: "28px", borderRadius: "50%",
                border: `1px solid ${ABR}`, background: ABG,
                display: "flex", alignItems: "center", justifyContent: "center",
              ...
              {userOpen && (
                <div style={{ ...dropStyle, right: 0, minWidth: "180px" }}>
                  <div style={{ padding:"8px 12px 10px",borderBottom:`1px solid ${DROPB}`,marginBottom:"3px" }}>
                    <div style={{ fontSize:"12px",fontWeight:500,color:TEXT,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{session.user?.name}</div>
                    <div style={{ fontSize:"11px",color:MUTED,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{session.user?.email}</div>
                  </div>
                  {[
                    ...
                    <Link key={l.href} href={l.href} style={{ display:"block",padding:"7px 12px",borderRadius:"7px",textDecoration:"none",color:TEXT,fontSize:"12px",fontWeight:300 }}
                      onMouseEnter={e=>e.currentTarget.style.background=DH}
                  ...
                  <button onClick={()=>signOut({callbackUrl:"/"})} style={{
                    ...
                    padding:"7px 12px",borderRadius:"7px",cursor:"pointer",fontSize:"12px",
                    textAlign:"left",marginTop:"2px",borderTop:`1px solid ${DROPB}`,
          ...
          {/* Hamburger */}
          <button onClick={()=>setMenu(!menuOpen)} className="nb" style={{
            width:"28px",height:"28px",borderRadius:"50%",
            border:`1px solid ${PILLB}`,background:"transparent",
        ...
        <div style={{
          ...
          {LINKS.map(l=>(
            <Link key={l.href} href={l.href} style={{
              ...
              color:(l as any).gold?"#B5863A":TEXT,
            }}>
              {l.label}
          ...
          {session && <>
            ...
            <button onClick={()=>signOut({callbackUrl:"/"})} style={{
              width:"100%",background:"none",border:"none",color:"#ef4444",
              padding:"18px 0",fontSize:"17px",fontWeight:300,textAlign:"left",
